...
class DB:
    ...
    def insert(self, table_obj):
        self.database.append(table_obj)

    def search(self, search_name):
        for obj in self.database:
            if obj.table_name == search_name:
                return obj
...
class Table:
    ...
    def filter(self, condition):
        ...
        for i in self.table:
            if condition(i):
                temp.append(i)
        return Table(f'{self.table_name}_filtered', temp)
    ...
    def join(self, table_to_join, pivot):
        placeholder = []
        for item in self.table:
            for pivoter in table_to_join.table:
                if item[pivot] == pivoter[pivot]:
                    new_row = item.copy()
                    new_row.update(pivoter)
                    placeholder.append(new_row)
        return Table(self.table_name + "_joined", placeholder)
